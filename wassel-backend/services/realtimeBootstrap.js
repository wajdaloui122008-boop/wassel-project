...
if (express.application.__veltoRealtimePatched) return;
...
express.application.listen = function veltoRealtimeListen(...args) {
  ...
  io.on("connection", (socket) => {
    socket.on("order:watch", async (orderId) => {
      ...
      try {
        const order = await Order.findById(orderId).select("client livreur status serviceType pickupLocation dropoffLocation distanceKm estimatedDurationMin currency paymentStatus statusHistory");
        if (!order) return;
        const userId = String(socket.user?.id || "");
        if (String(order.client || "") !== userId && String(order.livreur || "") !== userId && socket.user?.role !== "admin") return;
        socket.join(`order:${orderId}`);
        socket.emit("order:snapshot", await buildSnapshot(order));
      } catch (err) {
  ...
  const port = args[0] == null ? process.env.PORT || 10000 : args[0];
  const callback = typeof args[args.length - 1] === "function" ? args[args.length - 1] : null;
  const listenArgs = callback ? args.slice(0, -1) : args;
  server.listen(...listenArgs, () => {
    console.log(`Velto realtime gateway actif sur le port ${port}`);
