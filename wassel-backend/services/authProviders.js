...
async function findOrCreate({ email, name, role }) {
...
router.get("/google", (req, res) => {
  ...
  const state = makeState({ provider: "google", role: roleFrom(req.query.role) });
  const url = new URL("https://accounts.google.com/o/oauth2/v2/auth");
  url.searchParams.set("client_id", clientId); url.searchParams.set("redirect_uri", `${baseUrl()}/auth/google/callback`); url.searchParams.set("response_type", "code"); url.searchParams.set("scope", "openid email profile"); url.searchParams.set("state", state);
...
router.get("/google/callback", async (req, res) => {
  try {
    ...
    const code = clean(req.query.code, 2000), clientId = clean(process.env.GOOGLE_CLIENT_ID, 300), clientSecret = clean(process.env.GOOGLE_CLIENT_SECRET, 500);
    ...
    const result = await findOrCreate({ email: payload.email, name: payload.name, role: state.role });
...
router.get("/apple", (req, res) => {
  ...
  const state = makeState({ provider: "apple", role: roleFrom(req.query.role) });
  const url = new URL("https://appleid.apple.com/auth/authorize");
  url.searchParams.set("client_id", clientId); url.searchParams.set("redirect_uri", `${baseUrl()}/auth/apple/callback`); url.searchParams.set("response_type", "code"); url.searchParams.set("response_mode", "form_post"); url.searchParams.set("scope", "name email"); url.searchParams.set("state", state);
  res.redirect(url.toString());
});
...
router.post("/phone/request", async (req, res) => {
  ...
  const response = await fetch(`https://verify.twilio.com/v2/Services/${process.env.TWILIO_VERIFY_SERVICE_SID}/Verifications`, { method: "POST", headers: { Authorization: `Basic ${auth}`, "Content-Type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ To: phone, Channel: "sms" }) });
...
router.post("/phone/verify", async (req, res) => {
  ...
  if (!process.env.TWILIO_ACCOUNT_SID || !process.env.TWILIO_AUTH_TOKEN || !process.env.TWILIO_VERIFY_SERVICE_SID) return res.status(503).json({ error: "La connexion par numéro nécessite la configuration SMS du serveur." });
  const auth = Buffer.from(`${process.env.TWILIO_ACCOUNT_SID}:${process.env.TWILIO_AUTH_TOKEN}`).toString("base64");
  const response = await fetch(`https://verify.twilio.com/v2/Services/${process.env.TWILIO_VERIFY_SERVICE_SID}/VerificationCheck`, { method: "POST", headers: { Authorization: `Basic ${auth}`, "Content-Type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ To: phone, Code: code }) });
